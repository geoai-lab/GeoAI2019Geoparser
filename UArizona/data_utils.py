...
class CoNLLDataset(object):
    ...
    def __iter__(self):
        niter = 0
        if self.processing_tag is None:
            self.processing_tag = lambda x: x if x in ['B-LOC','I-LOC'] else 'O'
...
def get_processing_word(vocab_words=None, vocab_chars=None,vocab_pref_suff=None,vocab_pref_suff_2=None,vocab_pref_suff_4=None,
                    lowercase=False, chars=False, Pref_Suff=False, Geoparser=False):
    """
    Args:
        vocab: dict[word] = idx
    Returns:
...
def createMatrices(sentences):
    ...
    for sentence in sentences:
        ...
        for word, label in sentence:
            ...
            words.append(word)
            indexes.append(int(label))
        dataset.append([words, indexes])
